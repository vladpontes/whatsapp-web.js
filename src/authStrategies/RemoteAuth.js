...
class RemoteAuth extends BaseAuthStrategy {
    constructor({ clientId, dataPath, store, backupSyncIntervalMs } = {}) {
        console.log('starting RemoteAuth constructor v2')
    ...
    async storeRemoteSession(options) {
        ...
        if (pathExists) {
            ...
            await fs.promises.unlink(path.join('/tmp', `${this.sessionName}.zip`));
    ...
    async extractRemoteSession() {
        const pathExists = await this.isValidPath(this.userDataDir);
        const compressedSessionPath = path.join('/tmp', `${this.sessionName}.zip`);
        const sessionExists = await this.store.sessionExists({ session: this.sessionName });
        if (pathExists) {
            await fs.promises.rm(this.userDataDir, {
                recursive: true,
    ...
    async compressSession() {
        ...
        //fixing writing on ready-only dir own serverless functions.
        const stream = fs.createWriteStream(path.join('/tmp', `${this.sessionName}.zip`));
